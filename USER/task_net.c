...
void OnServerHandle(void)
{
	...
	len = NetDataFrameHandle(&tcp,net_send_buf,HoldReg,ConnectState);	//读取并解析服务器下发的数据包
	...
	if(Usart1RecvEnd == 0xAA)
	{
		...
		if(Usart1FrameLen != 0)
		{
			net_send_len = Usart1FrameLen;

			memcpy(net_send_buf,Usart1RxBuf,net_send_len);
...
void SendHeartBeatPacket(void)
{
	...
	if(GetSysTick1s() - times_sec >= UpLoadINCL)	//只有在数据上传周期大于20秒的时候才发送心跳包，间隔是20秒
	{
		...
		send_len = PackNetData(cmd_id,NULL,0,send_buf);
	}
